...
/**
 * Converts a string to PascalCase with proper handling of icon naming
 * @param {string} input - Input string to convert (e.g. 'alerts_feedback-alert-circle')
 * @param {boolean} includeCategory - Whether to include category in the name
 * @returns {string} PascalCase string (e.g. 'AlertCircle' or 'AlertsFeedbackAlertCircle')
 */
function pascalCase(input, includeCategory = false) {
  // Split into category and name parts
  const parts = input.includes('-') ? input.split('-') : [input];
  const category = parts[0];

  // Get the icon name (everything after the category)
  let iconName;
  if (!includeCategory && parts.length > 1) {
    // For icon name without category, take all parts after the first hyphen
    iconName = parts.slice(1).join('-');
  } else {
    // For full name or no hyphens, use the entire input
    iconName = input;
  }

  let pascalName = iconName
    .toLowerCase()
  ...
  // Check if the simple name (includeCategory=false) starts with a digit
  if (!includeCategory && /^\d/.test(pascalName)) {
    pascalName = `Icon${pascalName}`; // Prefix with "Icon"
  }
  // Note: Full names (includeCategory=true) like 'Alerts01' seem to be handled correctly by default,
  // as the initial letter comes from the category.
...
async function processSvgFile(
  ...
  // Generate PascalCase component name WITHOUT category prefix
  const componentName = pascalCase(`${category}-${baseName}`, false);
  // Also store the full name with category for internal tracking
  const fullComponentName = pascalCase(`${category}-${baseName}`, true);
  ...
  try {
    ...
    manifest.push({ 
      name: componentName, 
      category,
      fullName: fullComponentName // Add full name with category for reference
    });
...
${manifest.map(icon => `  '${icon.fullName}': '${icon.name}'`).join(',\n')}
};
`;
    ...
    const sortedReactExports = [...new Set(reactExports)].sort();
    const uniqueReactExportsByName = [];
    const seenReactNames = new Set();
    for (const exp of sortedReactExports) {
        const match = exp.match(/export \{ default as (\w+) \}/);
        if (match) {
            const name = match[1];
            if (!seenReactNames.has(name)) {
                uniqueReactExportsByName.push(exp);
                seenReactNames.add(name);
            } else {
                // Log the duplicate being skipped
                console.warn(`    ⚠️ Skipping duplicate simple export for '${name}' in React (keeping first encountered path).`);
            }
        } else {
             // Should not happen, but include the original export if regex fails
             uniqueReactExportsByName.push(exp);
        }
    }

    // Filter unique RN exports similarly
    const sortedRnExports = [...new Set(rnExports)].sort();
    const uniqueRnExportsByName = [];
    const seenRnNames = new Set();
     for (const exp of sortedRnExports) {
        const match = exp.match(/export \{ default as (\w+) \}/);
        if (match) {
            const name = match[1];
            if (!seenRnNames.has(name)) {
                uniqueRnExportsByName.push(exp);
                seenRnNames.add(name);
            } else {
                 console.warn(`    ⚠️ Skipping duplicate simple export for '${name}' in RN (keeping first encountered path).`);
            }
        } else {
             uniqueRnExportsByName.push(exp);
        }
    ...
    // Create backward compatibility aliases by generating re-exports
    const reactAliasExports = manifest.map(
      icon => `export { default as ${icon.fullName} } from './icons/${icon.category}/${icon.name}';`
    );
    
    const rnAliasExports = manifest.map(
      icon => `export { default as ${icon.fullName} } from './icons/${icon.category}/${icon.name}';`
    );

    const reactIndexContent = `// Auto-generated by generate-icons.js script @ ${new Date().toISOString()}
// Simple icon exports (preferred)
${uniqueReactExportsByName.join('\n')} // Use the filtered list

// Legacy alias exports (for backward compatibility)
${reactAliasExports.join('\n')}
`;
...
`;
    ...
    console.log(
      `⚛️ React components generated (incl. fallbacks): ${uniqueReactExportsByName.length}`,
    );
    console.log(
      `📱 React Native components generated (incl. fallbacks): ${uniqueRnExportsByName.length}`,
    );
